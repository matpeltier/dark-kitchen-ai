...
function issue(number: number, state: "OPEN" | "CLOSED" = "OPEN", labels = ["factory:auto"], blockedBy: number[] = []): GitHubIssue {
...
describe("GitHub issue graph", () => {
  ...
  it("waits for open blockers and becomes ready after a blocker closes", () => {
    const blocked = buildIssueGraph([issue(1), issue(2, "OPEN", ["factory:auto"], [1])]);
    expect(computeReadyIssues(blocked).map((item) => item.number)).toEqual([1]);
    const ready = buildIssueGraph([issue(1, "CLOSED"), issue(2, "OPEN", ["factory:auto"], [1])]);
  ...
  it("requires all blockers, excludes human/non-auto issues, and detects cycles", () => {
    const graph = buildIssueGraph([
      issue(1, "OPEN", ["factory:auto"], [2]),
      issue(2, "OPEN", ["factory:auto"], [1]),
      issue(3, "OPEN", ["factory:needs-human", "factory:auto"]),
      issue(4, "OPEN", []),
    ]);
  ...
  it("surfaces a closed dependency that was not Factory-managed", () => {
    const graph = buildIssueGraph([issue(1, "CLOSED", []), issue(2, "OPEN", ["factory:auto"], [1])]);
